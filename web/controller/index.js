...
obj.getSearchList=function (req, res, next) {
	 ...
	 // 多表查询
	  ...
	  var selectquery = 'select  a.id,a.user_id,a.title,a.source,a.source_link,a.time,a.update_time,a.point_num,a.img,a.description,b.user_img,b.username,c.clas_name';
	  if(query.uid){//如果传用户ID；就查那个用户发布的文章/帖子
	  	    if(req.session[__webUserInfo__]&&query.id==req.session[__webUserInfo__].id){//如果这个用户是当前登录的用户则查全部；
	  	    	   var getArtList_sql = selectquery+' from z_articles_list a inner join z_member b on a.user_id=b.id inner join z_articles_clas c on a.art_clas_id=c.id where concat(a.title,a.content) like "%'+keyword+'%" and a.user_id = '+query.uid+' order by time DESC limit '+page+','+limitCount;
	  	    }else{//如果这个用户不是当前登录的用户则查 有效的文章/帖子
	  	    	   var getArtList_sql = selectquery+' from z_articles_list a inner join z_member b on a.user_id=b.id inner join z_articles_clas c on a.art_clas_id=c.id where concat(a.title,a.content) like "%'+keyword+'%" and a.is_valid = 1 and a.user_id = '+query.uid+' order by time DESC limit '+page+','+limitCount;
	  	    }
	  }else{//如果 没有；则查全部
	  	    var getArtList_sql = selectquery+' from z_articles_list a inner join z_member b on a.user_id=b.id inner join z_articles_clas c on a.art_clas_id=c.id where concat(a.title,a.content) like "%'+keyword+'%" and a.is_valid = 1 order by time DESC limit '+page+','+limitCount;
	  }
...
obj.getArtList=function (req, res, next) {
	  ...
	  // var clas_sql = "select * from z_articles_comment where art_list_id = " + query.id + ' order by time limit '+page+','+limitCount;
	 // 多表查询
	  var selectquery = 'select  a.id,a.user_id,a.source,a.source_link,a.time,a.update_time,a.point_num,a.img,a.description,b.user_img,b.username,c.clas_name';
	  if(query.uid){//如果传用户ID；就查那个用户发布的文章/帖子
