...
window.Win10 = {
    ...
    // 新增：openUrlTitle
    // 主要作用：(主要 页面菜单 针对 菜单栏菜单 )
    // 根据下边菜单检查这个菜单是否点击过

    openUrlTitle:function(title){
         var listTitle=$("#win10_btn_group_middle .btn .btn_title span.title");
         for(var i=0;i<listTitle.length;i++){
              if(listTitle[i].innerHTML==title){
                    // console.log(listTitle[i].parentNode.parentNode.className)
                    if(listTitle[i].parentNode.parentNode.className.indexOf('active')==-1){
                          listTitle[i].parentNode.parentNode.click()
                    }
                    return true;
              }
         }
    ...
    // callback
    // 
    openUrl: function (url,img, title,areaAndOffset,callback) {
        // 

        if(Win10.openUrlTitle(title)){
              return false;
        }
        title=img+'<span class="title">'+title+'</span>';//修改 新增img 和span标签

        if(this._countTask>10){
            layer.msg("您打开的太多了，关掉一些吧~");
            return false;
        }else{
            this._countTask++;
        }
